...
async def mistral_insight(f: dict) -> str | None:
    ...
    facts_txt = "\n".join(lines) + "\n"
    system = (
        "Ты — аналитик процессов разработки в VkusVill. Опирайся на внутреннюю практику и особенности данных ниже "
        "и на сухие факты о блокировках. Дай короткий разбор на русском (3–4 предложения), строго в формате:\n"
        "Маркер: что в планировании/процессе, вероятно, пошло не так (например, зависимость не согласовали заранее, "
        "не выстроена последовательность задач, не соблюдён SLA). Используй данный «Характер главной причины» — "
        "не меняй внутреннюю на внешнюю. Для внутренней причины не упоминай внешние команды.\n"
        "Рекомендация: один конкретный следующий шаг в духе практики, согласованный с характером причины "
        "(внутренняя → внутреннее планирование/приоритеты/загрузка + эксперимент «Мы полагаем, что… приведёт к…»; "
        "внешняя → эскалация и разбор несоблюдения SLA). SLA уже существует — не предлагай «ввести SLA».\n"
        "Опирайся ТОЛЬКО на приведённые факты, не выдумывай числа. Без приветствий и воды.\n"
        "ТОН: пиши просто и по-человечески, как будто объясняешь коллеге за кофе. Короткие живые фразы. "
        "Никакого канцелярита и штампов — НЕ используй слова вроде «проактивно», «точки контроля», "
        "«команда-донор», «фиксация причин задержек», «цепочка поставки», «синхронизация подразделений». "
        "Говори по-простому: «договоритесь с командой X», «у вас не хватает людей», «задачи мешают друг другу».\n\n"
        + PRACTICE_BRIEF + "\n\n" + DOMAIN_NOTES)
