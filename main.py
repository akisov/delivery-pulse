...
# ── Радар практик ТехВилл ───────────────────────────────────────────────────────
RADAR_URL = "https://practice-radar.svc.vkusvill.ru/"
# Каталог практик (из радара). ctx — где уместно советовать. Ссылки — на радар целиком.
PRACTICES = [
    {"n": "Анализ блокировок",        "ctx": ["blockings", "sle", "arch", "flow"], "b": "разбирать блокеры (внутр/внеш), приоритет по суммарному времени, устранять корневую причину, SLA с внешними"},
    {"n": "WIP-лимит",                "ctx": ["flow", "blockings", "sle"],         "b": "ограничивать одновременную работу — фокус и скорость потока; превышение WIP растит возраст задач"},
    {"n": "Анализ метрик потока",     "ctx": ["flow", "blockings"],                "b": "следить за WIP Age / временем цикла / пропускной способностью, ловить узкие места и стареющие задачи"},
    {"n": "Визуализация работы",      "ctx": ["flow", "blockings", "arch"],        "b": "делать поток и блокеры видимыми на доске — проблемы всплывают раньше"},
    {"n": "Дейли митинг",             "ctx": ["flow"],                             "b": "ежедневная синхронизация по потоку: что мешает, где застряло"},
    {"n": "Оценка",                   "ctx": ["est", "sle"],                       "b": "осознанная оценка (SP/PERT/T-Shirt) для прогнозируемости и планирования"},
    {"n": "Уточнение бэклога (PBR)",  "ctx": ["sle", "arch", "est"],               "b": "прорабатывать задачи до входа в работу (критерии, MMF, риски) — меньше возвратов и переоценок"},
    {"n": "User Story Splitting",     "ctx": ["sle", "arch", "est"],               "b": "нарезать крупные истории на тонкие вертикальные срезы — быстрее проходят, реже нарушают SLE"},
    {"n": "Анализ требований",        "ctx": ["arch", "sle"],                      "b": "прорабатывать сценарии, негативные кейсы, ADS/ADR — меньше возвратов от арх.кома/ТА"},
    {"n": "Ретроспектива команды",    "ctx": ["flow", "incidents", "blockings"],   "b": "регулярно улучшать процесс по данным метрик"},
    {"n": "Код-ревью",                "ctx": ["arch", "incidents"],                "b": "обязательное ревью изменений — качество и меньше дефектов"},
    {"n": "Документирование баг-репортов", "ctx": ["incidents"],                   "b": "полные баг-репорты (шаги/ожид-факт/окружение) — быстрее чинить инциденты"},
    {"n": "Оценка рисков релизов и регрессионное тестирование", "ctx": ["incidents"], "b": "оценивать риск релиза и покрывать регрессом — меньше инцидентов после выката"},
    {"n": "Постмортемы",              "ctx": ["incidents"],                        "b": "безвинный разбор инцидента с корневой причиной и действиями — чтобы не повторялось"},
    {"n": "Логирование и трейсы",     "ctx": ["incidents"],                        "b": "достаточные логи/трейсы для быстрой диагностики"},
    {"n": "Отслеживание SLO и Error Budget", "ctx": ["incidents", "sle"],          "b": "держать SLO и бюджет ошибок, приоритизировать надёжность по данным"},
    {"n": "Здоровье продукта/сервиса","ctx": ["incidents"],                        "b": "мониторинг ключевых метрик сервиса — раннее обнаружение проблем"},
...
def _practices_prompt(ctx) -> str:
    ...
    return ("\n\nПРАКТИКИ ТехВилл (из «Радара практик»), которые уместно посоветовать по этому разделу — "
            "порекомендуй 1-2 НАИБОЛЕЕ подходящие СТРОГО из этого списка, назови точным названием "
            "(без ссылок — их добавит интерфейс):\n" + lines)

# Общее правило для всех AI-выводов: не только негатив.
AI_BALANCE_NOTE = ("\n\nБАЛАНС: обрабатывай и ПОЗИТИВ, не только проблемы. Если по числам есть улучшение "
                   "(стало меньше/быстрее/закрыли больше/динамика лучше прошлого периода) — обязательно отметь "
                   "это отдельным пунктом с ✅. Будь честен: если картина в целом здоровая — так и скажи, "
                   "не выдумывай проблему ради проблемы.")
...
ARCH_ISSUE_TYPES = ["story", "analytics", "technicaldebt", "improvement", "technicalimprovement", "elaboration"]
...
async def query_arch_dashboard(date_from: str, date_to: str, queues: list[str]):
    ...
    # родители возвратов, которых нет в arch_tasks (напр. тип не синкался) — тянем живьём
    still = [p for p in ret_by_parent if p and p not in tmap]
    if still and TRACKER_TOKEN:
        try:
            async with httpx.AsyncClient(timeout=30) as _c:
                fetched = await asyncio.gather(
                    *[tracker_request(_c, "GET", f"/v2/issues/{k}") for k in still],
                    return_exceptions=True)
            for k, iss in zip(still, fetched):
                if not isinstance(iss, dict):
                    continue
                title = iss.get("summary") or "—"
                q = (iss.get("queue") or {}).get("key") or k.split("-")[0]
                if arch_is_test_task(title) or q not in queues:
                    continue
                it = iss.get("type") or {}
                tmap[k] = {
                    "key": k, "title": title, "url": f"https://tracker.yandex.ru/{k}", "queue": q,
                    "issueType": it.get("key") or "story", "issueTypeDisplay": it.get("display") or "—",
                    "entryDates": [], "v1Dates": [], "v2Dates": [],
                }
        except Exception as e:
            print(f"[arch returns live] {e}")
...
async def mistral_insight(f: dict) -> str | None:
    ...
    system = (
        ...
        "Практика: одна подходящая практика из списка ниже (точным названием) — что она даст в этом случае.\n"
        ...
        + PRACTICE_BRIEF + "\n\n" + DOMAIN_NOTES + _practices_prompt("blockings") + AI_BALANCE_NOTE)
    return await ai_cached("blk", system, facts_txt, max_tokens=360, temperature=0.25)
...
async def incidents_ai(team: str = Query("all"), months: int = Query(12),
    ...
    system = (
        ...
        "— ПОСЛЕДНИМ пунктом добавь 📌 с рекомендацией 1 подходящей практики из списка ниже (точным названием) — что она даст.\n"
        "Только на основе чисел, ничего не выдумывай."
        + _practices_prompt("incidents") + AI_BALANCE_NOTE
    )
    summary = await ai_cached("inc", system, facts, max_tokens=430, temperature=0.3, refresh=refresh)
...
async def _osp_ai_summary_build(team: str, month: str) -> str | None:
    ...
    lines = [
        ...
        f"ЗАКРЫТО (сделано) задач всего: {d(dM.get('total'), dP.get('total'))}.",
        "  по типам (закрыто): " + ", ".join(f"{cat_lbl[k]} {d(dM.get(k), dP.get(k))}" for k in cat_lbl),
        f"Инцидентов ЗАВЕДЕНО (создано) за месяц: {d(iM, iP)}.",
        f"Инцидентов ЗАКРЫТО за месяц: {d(dM.get('incident'), dP.get('incident'))}. "
        f"ВАЖНО: сравни заведено vs закрыто — если закрыто БОЛЬШЕ заведённого, команда разгребла накопленные инциденты (это ПОЗИТИВ ✅).",
    ...
    system = (
        ...
        "за отчётный месяц и сравнение с предыдущим. Подсветь 2–4 ГЛАВНЫХ изменения для продакта — "
        "СБАЛАНСИРОВАННО: и ХОРОШЕЕ, и тревожное. Не пиши только про негатив.\n"
        "ОБЯЗАТЕЛЬНО начни с позитива, если он есть: закрыли БОЛЬШЕ задач всего, чем в прошлом месяце; "
        "меньше инцидентов; разгребли техдолг. Потом — тревожное.\n"
        "Примеры хорошего (✅): закрыли больше задач; инцидентов стало меньше; вложились в техдолг/тех.улучшения "
        "(это инвестиция в качество, а не всегда плохо).\n"
        "ИНЦИДЕНТЫ: строго различай ЗАВЕДЕНО (создано) и ЗАКРЫТО. Если ЗАКРЫТО больше, чем ЗАВЕДЕНО — "
        "команда И получила меньше новых инцидентов, И разгребла накопленные хвосты — это сильный позитив, "
        "отметь ✅ и назови оба числа. НЕ путай эти два показателя.\n"
        "Примеры тревожного: инцидентов завели больше, а закрыли меньше; выросли дни блокировок; "
        "перекос в инциденты в ущерб Story.\n"
        "ВАЖНО: рост часов на техдолг/тех.улучшения сам по себе НЕ негатив — это может быть плановое "
        "разгребание; оценивай в связке с объёмом закрытого и инцидентами.\n"
        "ФОРМАТ СТРОГО:\n"
        ...
        + _practices_prompt(["blockings", "incidents", "flow"]) + AI_BALANCE_NOTE
    )
...
# Полные имена (для фолбэка, если человек ничего не списал — иначе видно только фамилию).
COURIER_FULLNAMES = {
    "Туралиева": "Раушан Туралиева", "Перевезенцева": "Ольга Перевезенцева",
    "Доронин": "Сергей Доронин", "Егоров": "Роман Егоров", "Подлинов": "Владислав Подлинов",
    "Гусев": "Иван Гусев", "Драгун": "Юлия Драгун", "Источников": "Роман Источников",
    "Ким": "Андрей Ким", "Спиридонов": "Михаил Спиридонов", "Степин": "Олег Степин",
    "Корякин": "Владислав Корякин", "Копосов": "Евгений Копосов", "Резенова": "Полина Резенова",
    "Асотикова": "Светлана Асотикова", "Яцушко": "Максим Яцушко",
    "Памшев": "Кирилл Памшев", "Исабаев": "Бектур Исабаев", "Борискин": "Алексей Борискин",
    "Разумова": "Юлия Разумова", "Рогова": "Анастасия Рогова", "Киреев": "Александр Киреев",
    "Шестопалов": "Матвей Шестопалов", "Ву": "Александр Ву", "Мартынов": "Михаил Мартынов",
    "Махмутова": "Гюзель Махмутова",
}
SLACKER_NAMES = {}   # норм-фамилия → полное имя (фолбэк, если человек ничего не списал)
for _t, _ns in COURIER_TEAM_MEMBERS.items():
    for _n in _ns:
        SLACKER_NAMES[_cnorm(_n)] = COURIER_FULLNAMES.get(_n, _n)
...
async def slackers(refresh: bool = Query(False)):
    ...
    for p in people:
        if p["id"] in leave:
            on_leave.append({**p, "since": leave[p["id"]].get("since"), "kind": leave[p["id"]].get("kind", "отпуск")})
        elif p["last2"] < 8:
            slack.append(p)
...
async def _slacker_leave() -> dict:
    """{id(норм-фамилия): {name, team, label, kind, since}} — кто на отпуске/больничном."""
    snap = await _osp_snap("slackers-leave-v1")
    return snap if isinstance(snap, dict) else {}

@app.post("/slackers/leave")
async def slackers_leave(request: Request):
    """Тоггл «отпуск/больничный» по сотруднику: {id, on, name?, team?, label?, kind?}.
    Если on — сотрудник исключается из негодяев (он легально не вносит часы)."""
    try:
        body = await request.json()
    except Exception as e:
        return JSONResponse({"ok": False, "error": f"bad json: {e}"})
    pid = body.get("id")
    if not pid:
        return JSONResponse({"ok": False, "error": "нужен id"})
    leave = await _slacker_leave()
    if body.get("on"):
        leave[pid] = {"name": body.get("name"), "team": body.get("team"),
                      "label": body.get("label"), "kind": body.get("kind") or "отпуск",
                      "since": datetime.now(MSK).strftime("%Y-%m-%d")}
    else:
        leave.pop(pid, None)
    try:
        await turso_execute([stmt(
            "INSERT INTO osp_snapshot(which,data,updated_at) VALUES(?,?,datetime('now')) "
            "ON CONFLICT(which) DO UPDATE SET data=excluded.data, updated_at=excluded.updated_at",
            ["slackers-leave-v1", json.dumps(leave, ensure_ascii=False)])])
    except Exception as e:
        return JSONResponse({"ok": False, "error": str(e)})
...
async def home_weekly_ai(refresh: bool = Query(False)):
    ...
    system = (
        ...
        "Сделай короткую сводку-ВЫВОДЫ для руководителя. По каждому контуру 2-4 пункта. "
        "КАЖДЫЙ пункт — это ЛИБО «болит» ЛИБО «улучшение»:\n"
        "• 🔴 БОЛИТ (проблема/риск: рост инцидентов, перегруз WIP, долгие задачи, скрытые блокировки, «никто не работает», "
        "много возвратов) — ОБЯЗАТЕЛЬНО закончи конкретным «→ что делать» (шаг или практика).\n"
        "• 🟢 УЛУЧШЕНИЕ (позитивный тренд: закрыли больше, разгребли хвосты, меньше инцидентов/возвратов, быстрее цикл) — "
        "отметь и похвали.\n"
        "Сначала самое болезненное с действием, в конце — позитив. Где в фактах есть сравнение с прошлой неделей — "
        "обязательно отметь ДИНАМИКУ (📈 рост / 📉 спад / ≈ без изменений). "
        "ТОЧНЫЕ НАЗВАНИЯ показателей — не смешивай их и не придумывай общих ярлыков: «Учёт часов» (кто недосписал), "
        "«Арх.комитет» (возвраты), «Инциденты» (заведено/закрыто), «Поток» (WIP/возраст задач), «SLE» (риски PUTKURERA). "
        "НЕЛЬЗЯ называть инциденты «потоком» — это разные вещи. Пиши по одному пункту на показатель. "
