...
DOMAIN_NOTES = """Важные особенности данных (учитывай обязательно):
- Поле «Причина блокировки» в этом процессе заполняется ВСЕГДА. НЕ делай вывод, что причины не фиксируются, что нет культуры ретроспектив или что процесс не отслеживает причины — это неверно.
- Причина «Причина не известна» (и «Не указана») на практике чаще всего означает «Нет рук» — нехватку свободных исполнителей/ресурсов в команде. Если это топ-причина, трактуй её именно так и давай рекомендации про ресурсы и планирование загрузки (а не про «фиксацию причин»)."""
...
async def mistral_insight(f: dict) -> str | None:
    ...
    system = (
        ...
        "нет проверки готовности перед этапом, нет SLA с внешней командой). Отнеси главную причину к внутренней или внешней.\n"
        "Рекомендация: один конкретный следующий шаг в духе практики (например, «5 почему» по топ-причине, "
        "эксперимент по формуле «Мы полагаем, что… приведёт к…», или договориться об SLA).\n"
